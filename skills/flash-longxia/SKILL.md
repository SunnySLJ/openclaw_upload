---
name: flash-longxia
description: Use this skill when the user wants to generate a video from a static image through the zhenlongxia workflow in this repo, including image upload, image-to-text prompt generation, generateVideo submission, polling, and MP4 download.
---

# flash-longxia

将静态图片提交到真龙虾工作流，生成并下载视频。
...
3. 先校验提示词是否生成成功
4. 只有成功后才调用 `/api/v1/aiMediaGenerations/generateVideo`
5. 轮询 `/api/v1/aiMediaGenerations/getById`
6. 下载 MP4 到 `output/`
...
- 视频失败：检查 `generateVideo` 返回的任务信息和 `getById` 状态
- 轮询超时：默认等待 30 分钟，可在 `config.yaml` 调整
